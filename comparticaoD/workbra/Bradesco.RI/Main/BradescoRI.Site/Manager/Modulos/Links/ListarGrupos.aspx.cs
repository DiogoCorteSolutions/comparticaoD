...
public partial class Modulos_Links_ListarGrupos : System.Web.UI.Page
{
    ...
    {
        if (listPager.DataSource != null && listPager.DataSource is IEnumerable<GrupoLinks>)
        {
            IEnumerable<GrupoLinks> dados = (IEnumerable<GrupoLinks>)listPager.DataSource;
    ...
    {
        Response.Redirect("SalvarGrupo.aspx");
    }
    ...
    private void LerDados()
    {
        try
        {
            List<GrupoLinks> objDados = null;

            objDados = DOModLinks.ListarGrupos();
               ...
               item.ItemType == ListItemType.AlternatingItem)
                {
                    ...
                    if (chk.Checked)
                    {
                        try
                        {
                            DOModLinks.ExcluirGrupo(Convert.ToInt32(item.Cells[1].Text));
                        }
                        catch (Exception ex)
                        {
