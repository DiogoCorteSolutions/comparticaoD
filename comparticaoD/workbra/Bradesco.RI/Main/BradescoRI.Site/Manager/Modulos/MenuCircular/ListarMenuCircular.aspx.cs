...
public partial class Modulos_MenuCircular_ListarMenuCircular : System.Web.UI.Page
{
    ...
    {
        if (listPager.DataSource != null && listPager.DataSource is IEnumerable<MenuCircular>)
        {
            IEnumerable<MenuCircular> dados = (IEnumerable<MenuCircular>)listPager.DataSource;
            ...
            switch (e.SortExpression)
            {
                case "menuCircularId":
                    if (AscendingSort)
                        dados = dados.OrderBy(u => u.IdMenuCircular).ToArray();
                    else
                        dados = dados.OrderByDescending(u => u.IdMenuCircular).ToArray();
                    break;
                case "Titulo":
                    if (AscendingSort)
    ...
    {
        Response.Redirect(string.Format("EditarMenuCircular.aspx?Grupo={0}", Request.QueryString["Grupo"].ToString()));
    }
               ...
               item.ItemType == ListItemType.AlternatingItem)
                {
                    ...
                    if (chk.Checked)
                    {
                        try
                        {
                            DOModMenuCircular.ExcluirMenuCircular(Convert.ToInt32(item.Cells[1].Text), Convert.ToInt32(item.Cells[2].Text), Convert.ToInt32(item.Cells[3].Text));
                        }
                        catch (Exception ex)
                        {
