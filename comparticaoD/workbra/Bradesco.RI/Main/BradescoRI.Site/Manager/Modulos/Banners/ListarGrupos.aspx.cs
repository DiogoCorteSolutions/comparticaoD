...
public partial class Modulos_Banner_ListarGrupos : System.Web.UI.Page
{
    ...
    private void LerDados()
    {
        try
        {
            List<GrupoBanners> objDados = null;

            objDados = DOModBanners.ListarGrupos();
               ...
               item.ItemType == ListItemType.AlternatingItem)
                {
                    ...
                    if (chk.Checked)
                    {
                        try
                        {
                            DOModBanners.ExcluirGrupo(Convert.ToInt32(item.Cells[1].Text));
                        }
                        catch (Exception ex)
                        {
