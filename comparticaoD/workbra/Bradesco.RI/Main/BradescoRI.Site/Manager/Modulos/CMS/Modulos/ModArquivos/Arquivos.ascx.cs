...
public partial class Modulos_CMS_Modulos_ModArquivos_Arquivos : System.Web.UI.UserControl
{
    ...
    protected void ddlAno_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            else if (rptDownloadVideos.Visible)
    ...
    protected void rptDownloadVideos_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        try
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                var capa = DOArquivos.Obter(new Arquivos() { Id = item.CapaId });
    ...
    private int BindArquivos(Repeater objRepeater)
    {
        ...
        try
        {
            ...
            lstSearch = VerificaArquivos(lstSearch.Where(x => x.CapaId <= 0).ToList());
    ...
    private void CarregarDestaqueVideos(List<Arquivos> lstItens)
    {
        try
        {
            if (lstItens != null)
            {
                ...
                if (arq != null)
                {
                    var arqDestaque = DOArquivos.Obter(new Arquivos() { Id = arq.CapaId });
    ...
    protected void lnkDownload_Command(object sender, CommandEventArgs e)
    {
        ...
        {
            Response.Clear();
            Response.ContentType = PegarExtensaoArquivo(e.CommandArgument.ToString()); // "application/octet-stream";
            Response.AddHeader("Content-Disposition", "attachment; filename=" + e.CommandArgument.ToString());
            Response.WriteFile(Server.MapPath(e.CommandArgument.ToString()));
            Response.End();
        }
    }
