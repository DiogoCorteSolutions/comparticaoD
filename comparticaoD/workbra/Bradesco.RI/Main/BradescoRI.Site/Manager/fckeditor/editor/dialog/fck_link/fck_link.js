...
function LoadAnchorNamesAndIds()
{
	...
	var oMenuCircular = oEditor.FCK.EditorDocument.getElementsByTagName( 'A' ) ;
	for( i = 0 ; i < oMenuCircular.length ; i++ )
	{
		if ( oMenuCircular[i].name && ( oMenuCircular[i].name.length > 0 ) )
			aAnchors[ aAnchors.length ] = oMenuCircular[i] ;
	}
...
function Ok()
{
	...
	var aMenuCircular = oLink ? [ oLink ] : oEditor.FCK.CreateLink( sUri, true ) ;

	var aHasSelection = ( aMenuCircular.length > 0 ) ;
	if ( !aHasSelection )
	{
		sInnerHtml = sUri;
		...
		aMenuCircular = [ oEditor.FCK.InsertElement( 'a' ) ] ;
	}

	for ( var i = 0 ; i < aMenuCircular.length ; i++ )
	{
		oLink = aMenuCircular[i] ;
	...
	oEditor.FCKSelection.SelectNode( aMenuCircular[0] );
