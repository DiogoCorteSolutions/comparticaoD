...
public partial class Modulos_Links_ListarLinks : System.Web.UI.Page
{
    ...
    {
        if (listPager.DataSource != null && listPager.DataSource is IEnumerable<Links>)
        {
            IEnumerable<Links> dados = (IEnumerable<Links>)listPager.DataSource;
            ...
            switch (e.SortExpression)
            {
                case "IdLink":
                    if (AscendingSort)
                        dados = dados.OrderBy(u => u.IdLink).ToArray();
                    else
                        dados = dados.OrderByDescending(u => u.IdLink).ToArray();
                    break;
                case "Titulo":
                    if (AscendingSort)
                ...
                case "URL":
                    if (AscendingSort)
                        dados = dados.OrderBy(u => u.Url).ToArray();
                    else
                        dados = dados.OrderByDescending(u => u.Url).ToArray();
                    break;
    ...
    {
        Response.Redirect(string.Format("EditarLinks.aspx?Grupo={0}", Request.QueryString["Grupo"].ToString()));
    }
    ...
    private void IniciaTela()
    {
        try
        {
            ...
            //Permissão de edição
            this.grdDados.Columns[6].Visible = ((Modulos_Modulos)Master).VerificaPermissaoEdicao();
    ...
    private void LerDados()
    {
        try
        {
            List<Links> objDados = null;

            objDados = DOModLinks.ListarLinks(Convert.ToInt32(Request.QueryString["Grupo"]), Convert.ToInt32(ddlIdioma.SelectedValue));
               ...
               item.ItemType == ListItemType.AlternatingItem)
                {
                    ...
                    if (chk.Checked)
                    {
                        try
                        {
                            DOModLinks.ExcluirLink(Convert.ToInt32(item.Cells[1].Text), Convert.ToInt32(item.Cells[2].Text), Convert.ToInt32(item.Cells[3].Text));
                        }
                        catch (Exception ex)
                        {
