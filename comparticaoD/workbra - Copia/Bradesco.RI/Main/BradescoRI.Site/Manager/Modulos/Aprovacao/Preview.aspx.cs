...
public partial class Modulos_Aprovacao_Preview : System.Web.UI.Page
{
    ...
    private void Publicar()
    {
        ...
        urlPagina = String.Concat(urlPagina, "?AprovacaoId=", Utilitarios.EnCryptDecrypt.CryptorEngine.Encrypt(aprovacaoId.ToString()), "&Aprovar=", Utilitarios.EnCryptDecrypt.CryptorEngine.Encrypt("True"));
                        ...
                        if (objConteudoPagina.Arquivo.ToLower().Contains("header"))
                        {
                            ...
                            if (publicar && objConteudoPagina.ModuloDinamico)
                            {
                                litConteudoPublicar = new LiteralControl
                                {
                                    ...
                                    Text = string.Concat("<ContDinamico:CarregaConteudoDinamico runat='server' IdModulo='", objConteudoPagina.ModuloId, "' PathModulo='", objConteudoPagina.Arquivo, "' />")
                                };
    ...
    private string TrataConteudo(string conteudo, string MetatagsKeywords, string MetatagsDescription)
    {
        try
        {
            ...
            conteudo = string.Concat("<%@ Register TagPrefix='ContDinamico' Namespace='Manager.Controls' %>  ", conteudo);
        }
        catch (Exception ex)
        {
            ExibirAlerta(ex.ToString(), true);
