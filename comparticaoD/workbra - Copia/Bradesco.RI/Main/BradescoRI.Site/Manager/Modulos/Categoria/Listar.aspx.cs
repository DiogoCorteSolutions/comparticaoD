...
public partial class Modulos_CMS_Paginas_ListarArea : System.Web.UI.Page
{
    ...
    protected void grdDados_SortCommand(object source, DataGridSortCommandEventArgs e)
    {
        if (listPager.DataSource != null && listPager.DataSource is IEnumerable<Categoria>)
        {
            ...
            switch (e.SortExpression)
            {
                case "IdCategoria":
                    if (AscendingSort)
                        dados = dados.OrderBy(u => u.IdCategoria).ToArray();
                    else
                        dados = dados.OrderByDescending(u => u.IdCategoria).ToArray();
                    break;
                case "Descricao":
                    if (AscendingSort)
