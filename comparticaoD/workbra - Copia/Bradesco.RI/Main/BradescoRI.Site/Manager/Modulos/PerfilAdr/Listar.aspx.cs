...
public partial class Modulos_PerfilAdr_Listar : System.Web.UI.Page
{
    ...
    {
        if (listPager.DataSource != null && listPager.DataSource is IEnumerable<ModPerfilAdr>)
        {
            IEnumerable<ModPerfilAdr> dados = (IEnumerable<ModPerfilAdr>)listPager.DataSource;
            ...
            switch (e.SortExpression)
            {
                ...
                case "NomeIdioma":
                    if (AscendingSort)
                        dados = dados.OrderBy(u => u.NomeIdioma).ToArray();
                    else
                        dados = dados.OrderByDescending(u => u.NomeIdioma).ToArray();
                    break;
                case "SiglaIdioma":
                    if (AscendingSort)
                        dados = dados.OrderBy(u => u.SiglaIdioma).ToArray();
                    else
                        dados = dados.OrderByDescending(u => u.SiglaIdioma).ToArray();
                    break;
                default:
                    if (AscendingSort)
                        dados = dados.OrderBy(u => u.ID).ToArray();
    ...
    private void IniciaTela()
    {
        try
        {
            ...
            this.ddlIdioma.Items.Insert(0, new ListItem(Resources.Textos.Texto_Todos, "0"));
    ...
    private void LerDados()
    {
        try
        {
            List<ModPerfilAdr> objDados = null;

            objDados = DOModPerfilAdr.Listar(Convert.ToInt32(ddlIdioma.SelectedValue));
               ...
               item.ItemType == ListItemType.AlternatingItem)
                {
                    ...
                    if (chk.Checked)
                    {
                        try
                        {
                            DOModPerfilAdr.Excluir(Convert.ToInt32(item.Cells[1].Text));
                        }
                        catch (Exception ex)
                        {
