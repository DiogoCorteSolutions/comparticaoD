...
public class DOTimeline
{
    ...
    public static Timeline Inserir(Timeline item)
    {
        var strConectionString = ConfigurationManager.ConnectionStrings["BradescoRI"].ConnectionString;
        var objConexao = new SqlConnection(strConectionString);

        using (var objComando = new SqlCommand("SPE_I_Modulo_Timeline"))
        {
            objComando.Connection = objConexao;
            objComando.CommandType = CommandType.StoredProcedure;
            ...
            objComando.Parameters.Add("@TimelineId", SqlDbType.Int).Direction = ParameterDirection.Output;
            ...
            try
            {
                ...
                item.Id = int.Parse(objComando.Parameters["@TimelineId"].Value.ToString());
